# Simulated QA Test for BOLA Engine
$ErrorActionPreference = "Stop"

Write-Host "[*] Starting Proxy Engine on Port 8888..." -ForegroundColor Cyan
$proxyJob = Start-Process -FilePath "C:\Users\User\go\bin\bola.exe" -ArgumentList "proxy", "-c", "bola.yaml" -RedirectStandardOutput "proxy.log" -RedirectStandardError "proxy_err.log" -PassThru -NoNewWindow
Start-Sleep -Seconds 3

# Configuration from bola.yaml
...
try {
    $out1 = Invoke-WebRequest -Uri "https://api.dashboard.magic.link/v1/magic_client/r85h34hQg16SegSbk_3I5zh4r7ZIvfl9hwSU5KvMToE=/users" -Proxy "http://127.0.0.1:8888" -Headers @{ "Cookie" = $ownerCookie } -SkipCertificateCheck -Method Get -ErrorAction Stop
} catch {
...
try {
    $out2 = Invoke-WebRequest -Uri "https://api.dashboard.magic.link/v1/magic_client/attacker_client_id/users" -Proxy "http://127.0.0.1:8888" -Headers @{ "Cookie" = $attackerCookie } -SkipCertificateCheck -Method Get -ErrorAction Stop
} catch {
    Write-Host $_.Exception.Message -ForegroundColor Red
}
...
Stop-Process -Id $proxyJob.Id -Force

Write-Host "[*] Proxy Output Log:" -ForegroundColor Magenta
Get-Content proxy.log -ErrorAction SilentlyContinue
Get-Content proxy_err.log -ErrorAction SilentlyContinue
